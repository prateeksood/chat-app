    ...
    const element = DOM.create("div", {
      ...
      contextmenu: event => {
        ...
        menu.addItem("delete", "Delete", () => {
          App.request(`/message/${chatMessage.id}/delete`, {
            method: "GET"
          })
            .then(response => {
              const index = App.data.chats.get(message.chatId).messages.findIndex(message => message.id === chatMessage.id);
              if (index >= 0) {
                App.data.chats.get(message.chatId).messages.splice(index, 1);
                const deletedMessage = document.querySelector(`[c-id="${chatMessage.id}"]`);
                if (deletedMessage)
